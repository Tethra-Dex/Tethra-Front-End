...
import CanvasGridOverlay from '@/features/trading/components/overlays/CanvasGridOverlay';
import GridSettingsPanel from '@/features/trading/components/overlays/GridSettingsPanel';
import { DEFAULT_GRID_CONFIG } from '@/types/gridTrading';
...
const TradingVueChart: React.FC<TradingVueChartProps> = memo(({ symbol, interval }) => {
  ...
  useEffect(() => {
    ...
    const initializeChart = async () => {
      ...
      try {
        ...
        wsRef.current = pythDataFeed.createWebSocket(symbol, interval, (candle: Candle) => {
          if (chart) {
            const newCandle = {
              timestamp: candle.time,
              open: candle.open,
    ...
    if (chartContainerRef.current) {
      resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          console.log(
            'ResizeObserver detected size change:',
  ...
  useEffect(() => {
    ...
    if (!chartRef.current || isLoading) {
      console.log('⏳ Chart not ready or loading, skipping entry line draw', {
        isLoading,
    ...
    if (selectedPosition.symbol !== chartSymbolClean) {
      console.log(
        `❌ Symbol mismatch: position=${selectedPosition.symbol}, chart=${chartSymbolClean}`,
      );
      return;
    }
    ...
    console.log(
      `📍 Will draw entry price line at $${entryPrice.toFixed(2)} for ${
        isLong ? 'Long' : 'Short'
    ...
    const drawTimeout = setTimeout(() => {
      ...
      try {
        ...
        console.log(
          `✅ Successfully drew entry price line for ${
            selectedPosition.symbol
          } at $${entryPrice.toFixed(2)}`,
        );
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-2 left-2 z-20 flex flex-col gap-1"
      ...
      >
        ...
        {/* Grid Settings Panel */}
        {showGridSettings && gridTrading?.gridConfig?.enabled && gridTrading && (
          <GridSettingsPanel
        ...
        {gridTrading?.gridConfig?.enabled &&
          gridTrading?.stats &&
          gridTrading.stats.totalCells > 0 && (
      ...
      <div ref={chartContainerRef} className="w-full h-full" style={{ position: 'relative' }}>
        {/* Canvas Grid Overlay */}
        <CanvasGridOverlay
          chartRef={chartRef}
          gridConfig={gridTrading?.gridConfig || DEFAULT_GRID_CONFIG}
          selectedCells={gridTrading?.selectedCells || new Set()}
