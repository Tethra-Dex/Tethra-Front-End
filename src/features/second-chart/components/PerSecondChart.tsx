...
interface PerSecondChartProps {
  ...
  enableTapToTrade?: boolean; // Enable tap-to-trade open position mode
}
...
const PerSecondChart: React.FC<PerSecondChartProps> = ({
  ...
  enableTapToTrade = false,
}) => {
  const { placeBet, placeBetWithSession, isPlacingBet } = useOneTapProfit();
  const {
    isEnabled: isTapToTradeEnabled,
    tradeMode,
    handleCellClick: tapToTradeHandleCellClick,
    cellOrders,
  } = useTapToTrade();
  ...
  // Grid Y configuration - 0.006% of INITIAL price for all coins (proportional scaling)
  // Using initialPrice instead of currentPrice to keep Y-axis stable
  const GRID_Y_DOLLARS = (initialPrice > 0 ? initialPrice : currentPrice) * 0.00006; // 0.006% of initial price
  ...
  useEffect(() => {
    if (currentPrice > 0 && initialPriceRef.current === 0) {
      console.log('Setting initial price to:', currentPrice);
  ...
  useEffect(() => {
    ...
    const updateFocus = () => {
      ...
      const GRID_Y_DOLLARS_CALC = initialPrice * 0.005;
      const pixelsPerDollar = gridSizePixels / GRID_Y_DOLLARS_CALC;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'c':
          ...
          setIsFocusMode((prev) => {
            const newValue = !prev;
            console.log('Focus mode toggled:', newValue ? 'ON' : 'OFF');
            return newValue;
          });
          break;
      }
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let price = lowestPriceLevel; price <= highestPriceLevel; price += GRID_Y_DOLLARS) {
        ...
        // Determine decimals: if GRID_Y_DOLLARS < 0.01, show 3 decimals; if < 1, show 2 decimals; else 0
        const decimals = GRID_Y_DOLLARS < 0.01 ? 3 : GRID_Y_DOLLARS < 1 ? 2 : 0;
      ...
      // Round to nearest 1-second boundary for clean grid
  ...
  const handleMouseUp = useCallback(
    async (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (isDragging && !hasMoved && hoveredCell) {
        ...
        // Check if tap-to-trade open-position mode is active
        if (isTapToTradeEnabled && tradeMode === 'open-position') {
          // TAP-TO-TRADE MODE: Create limit order via context
          console.log('🎯 Tap-to-Trade mode: Creating limit order');

          // Parse cell coordinates from hoveredCell ID
          const parsed = hoveredCell.match(/^cell-(-?\d+)-(-?\d+)$/);
          if (!parsed) {
            console.error('Invalid cell ID format:', hoveredCell);
            return;
          }

          const cellX = parseInt(parsed[1], 10);
          const cellY = parseInt(parsed[2], 10);

          // Call tap-to-trade handler (creates order in backend)
          await tapToTradeHandleCellClick(cellX, cellY);
        } else {
          // ONE-TAP-PROFIT MODE: Place binary bet
          console.log('⚡ One-Tap-Profit mode: Placing binary bet');
