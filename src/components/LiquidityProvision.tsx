...
export default function LiquidityProvision({ className = '' }: LiquidityProvisionProps) {
  ...
  const formattedVaultAssets = vaultAssets ? Number(formatUnits(vaultAssets as bigint, 6)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00';
  const formattedUserUsdc = userUsdc ? Number(formatUnits(userUsdc as bigint, 6)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00';
  const formattedUserShares = userShares ? Number(formatUnits(userShares as bigint, VAULT_SHARE_DECIMALS)).toLocaleString('en-US', { minimumFractionDigits: 6, maximumFractionDigits: 6 }) : '0.000000';
  ...
  const minDepositForOneShareUnit = useMemo(() => {
    ...
    return ((vaultAssets as bigint) + (vaultSharesSupply as bigint) - 1n) / (vaultSharesSupply as bigint);
  ...
  return (
    <div className={`bg-slate-900/50 rounded-lg border border-slate-800 p-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <MetricCard label="Vault TVL" value={`$${formattedVaultAssets}`} icon={<TrendingUp className="text-green-400" size={18} />} />
        <MetricCard label="Your USDC" value={`${formattedUserUsdc} USDC`} icon={<Clock className="text-blue-400" size={18} />} />
        <MetricCard label="Your Vault Shares" value={`${formattedUserShares} vUSDC`} icon={<Droplets className="text-purple-400" size={18} />} />
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg border ${mode === 'deposit' ? 'border-blue-500 text-blue-200' : 'border-slate-700 text-gray-300'} bg-slate-800`}
          >
            Deposit
          </button>
          <button
            onClick={() => setMode('withdraw')}
            className={`px-4 py-2 rounded-lg border ${mode === 'withdraw' ? 'border-blue-500 text-blue-200' : 'border-slate-700 text-gray-300'} bg-slate-800`}
          >
            Withdraw
          </button>
        </div>
        ...
        >
          {isTxPending ? 'Pending...' : mode === 'deposit' ? (needsApproval ? 'Approve & Deposit' : 'Deposit') : 'Withdraw'}
        </button>
      </div>

      {mode === 'deposit' && minDepositForOneShareUnit && (
        <p className="text-xs text-gray-400 mt-2">
          Current min deposit to mint 1 share unit: ~{Number(formatUnits(minDepositForOneShareUnit, 6)).toFixed(6)} USDC
        </p>
      )}
      {error && <p className="text-xs text-red-400 mt-2">{error}</p>}

      {!isConnected && (
        <p className="text-xs text-yellow-400 mt-3">Connect your wallet to interact with the vault.</p>
      )}
    </div>
  );
}
